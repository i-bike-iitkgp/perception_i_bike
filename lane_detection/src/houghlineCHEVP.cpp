...
void Probabilistic_Hough( int, void* )
{
  ...
  for(int k=0;k<5;k++)
  {
   ...
   temp[k]=edges(Range(k*(edges.rows)/5,(k+1)*edges.rows/5),Range(0,edges.cols));
  ...
  for( size_t i = 0; i < p_lines.size(); i++ )
     {
       Vec4i l = p_lines[i];
       line( probabilistic_hough, Point(l[0], k*(edges.rows)/5+l[1]), Point(l[2], k*(edges.rows)/5+l[3]), Scalar(255,0,0), 3, CV_AA);
     }
   }
